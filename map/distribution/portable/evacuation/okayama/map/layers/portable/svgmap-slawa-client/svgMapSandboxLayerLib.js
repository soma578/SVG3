	...
	messaging = new InterWindowMessaging(
		{
			...
			setInitialSvgImage: async function (msg) {
				...
				await messaging.callRemoteFunc("replaceSvgImage", {
					svgImageXml: new XMLSerializer().serializeToString(window.svgImage),
				});
...
function setSvgImage(svgImageXml) {
	...
	// Existing elements can receive later mutations too. Give them stable IDs before
	// the initial document is mirrored so additions under defs/groups can be applied.
	for (const element of window.svgImage.querySelectorAll("*")) {
		if (!element.hasAttribute(CUSTOM_ID_ATTR)) {
