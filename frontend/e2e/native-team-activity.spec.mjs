...
test('CSV管理画面は地名の三段階選択だけで活動を追加できる', async ({ page }) => {
  await page.goto('/map/publishers/team-activity-csv/admin.html')
  await expect(page.locator('#status')).toContainText('現在配信中のCSVを表示しています')
  await page.locator('#prefectureQuery').fill('岡山')
  await expect(page.locator('#municipalityQuery')).toBeEnabled()
  await page.locator('#municipalityQuery').fill('岡山市北区')
  await expect(page.locator('#districtQuery')).toBeEnabled()
  await page.locator('#districtQuery').fill('駅元町')
